...
from sebs.gramine_native.function import GramineNativeFunction
from sebs.gramine_native.storage import Minio
...
class GramineNative(System):
    @staticmethod
    def name() -> str:
        return "gramine_native"

    def __init__(
        self,
        system_config: SeBSConfig,
        config: GramineNativeConfig,
        cache_client: Cache,
    ...
    def function_type() -> "Type[Function]":
        return GramineNativeFunction

    def get_storage(self, replace_existing: bool = False) -> PersistentStorage:
        if not hasattr(self, "storage"):
            if not self.config.resources.storage_config:
                raise RuntimeError(
                    "The gramine_native deployment is missing the configuration of pre-allocated storage!"
                )
    ...
    def is_configuration_changed(
        ...
        function = cast(GramineNativeFunction, cached_function)
    ...
    def enforce_cold_start(self, functions: List[Function], code_package: Benchmark):
        for function in functions:
            function = cast(GramineNativeFunction, function)
            function.stop()
