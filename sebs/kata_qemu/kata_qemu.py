...
from sebs.kata_qemu.function import KataQemuFunction
from sebs.kata_qemu.storage import Minio
...
class KataQemu(System):
    ...
    def get_storage(self, replace_existing: bool = False) -> PersistentStorage:
        if not hasattr(self, "storage"):
            if not self.config.resources.storage_config:
                raise RuntimeError(
                    "The kata_qemu deployment is missing the configuration of pre-allocated storage!"
                )
    ...
    def is_configuration_changed(self, cached_function: Function, benchmark: Benchmark) -> bool:
        ...
        function = cast(KataQemuFunction, cached_function)
    ...
    def enforce_cold_start(self, functions: List[Function], code_package: Benchmark):
        for function in functions:
            function = cast(KataQemuFunction, function)
            function.stop()
